...
#else // TODO: misses implementation for ARM
...
#if defined(__i386__) || defined(__x86_64__)
	if (!err) {
		uint32_t addressOffset = (uint32_t)((char*)escapeIsland - (char*)originalFunctionPtr - 5);
	...
	uint8_t		*instructionSizes )
{
	...
	for (index = 0;index < instructionCount;index += 1)
	{
		...
		{
			uint32_t offset = (uint32_t)((uintptr_t)originalFunction - (uintptr_t)escapeIsland);
